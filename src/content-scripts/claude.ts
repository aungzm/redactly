...
/**
 * Set up input redaction for Claude
 */
async function setupInputRedaction(): Promise<void> {
  ...
  if (inputElement.hasAttribute('contenteditable')) {
    log('Detected Claude TipTap contenteditable div');
    setupClaudeInterception(inputElement as HTMLDivElement);
  } else {
...
/**
 * Set up interception for Claude's TipTap contenteditable div
 */
function setupClaudeInterception(element: HTMLDivElement): void {
  ...
  document.addEventListener('click', (e: MouseEvent) => {
    ...
    if (submitButton) {
      ...
      log(`Button clicked - Text: "${textContent}", Redacted: "${result.text}", Rules applied: ${result.appliedRules.length}`);
  ...
  log('Claude interception setup complete');
}
      ...
      if (mutation.addedNodes.length > 0) {
        ...
        if (input && !input.dataset.redactlyInitialized) {
          ...
          if (input.hasAttribute('contenteditable')) {
            setupClaudeInterception(input as HTMLDivElement);
          }
        }
      }
    }
